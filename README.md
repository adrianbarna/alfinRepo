...
- **Actualizări**: două condiții, ambele obligatorii. Câmpul `version` trebuie incrementat la fiecare release în **ambele** locuri — `plugin.json`-ul plugin-ului și intrarea lui din `marketplace.json`. Detecția actualizărilor compară cu versiunea din intrarea de marketplace; dacă lipsește de acolo, clientul primește „Already up to date" la infinit, cu `Update` dezactivat. Și `Sync automatically` trebuie pornit la client, în meniul `···` al marketplace-ului — **nu vine pornit din oficiu**, iar fără el plugin-ul rămâne pe versiunea de la instalare la nesfârșit, fără avertisment. Meniul afișează `Synced commit`, util pentru diagnostic: comparați-l cu ultimul commit de pe GitHub.
